# SERGIO FRANCO, LINA OJEDA, DANIEL RUDAS
...
def mover(configuracion, posicion, direccion):
    nueva_configuracion = configuracion.copy()
    if 0 <= posicion + direccion < len(nueva_configuracion) and 'r' not in nueva_configuracion[posicion + direccion]:
        nueva_configuracion[posicion + direccion] = nueva_configuracion[posicion]
        nueva_configuracion[posicion] = 'p'
...
def contar_configuraciones_contador(configuracion_actual, forma_movimiento):
    ...
    for configuracion in configuracion_actual:
        for posicion in range(len(configuracion)):
            if "r" not in configuracion[posicion]:
                continue
            
            indice_forma = int(configuracion[posicion][1]) - 1
            
            if forma_movimiento[indice_forma] == 'I':
                nueva_configuracion = mover(configuracion, posicion, -1)
            elif forma_movimiento[indice_forma] == 'D':
                nueva_configuracion = mover(configuracion, posicion, 1)
            else:
                continue
            
            if nueva_configuracion:
                resultados.append(nueva_configuracion)
